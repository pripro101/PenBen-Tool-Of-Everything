...
# Run PrHacktool
def run_prhacktool():
    """Launches the PrHacktool menu in a new terminal or current terminal as fallback."""
    hackingtool_py = TOOLS_DIR / "hackingtool" / "hackingtool.py"

    if hackingtool_py.exists():
        try:
            # Try to run in a new terminal
            print("Running HackingTool in a new terminal...")
            subprocess.Popen(["gnome-terminal", "--", "bash", "-c", f"sudo python3 {hackingtool_py}; exec bash"])
        except Exception as e:
            # Fallback to running in the current terminal if the new terminal fails
            print(f"Error running in a new terminal: {e}. Running in the current terminal...")
            subprocess.check_call(["sudo", "python3", str(hackingtool_py)])
    else:
        print("hackingtool.py not found. Please install HackingTool first.")
...
def display_menu():
    ...
    print("15. Setup Tools (Clone and Install)")
    print("16. Exit")
